...
def plot_scatter(gt_tsts, pred_tsts, xlabel, ylabel, title, xlim, ylim, xy_text, tick_interval):
    ...
    if actual_p_value < 0.001:
        label_text = "$r = {:.2f}$".format(r_value)
    else:
        label_text = "$r = {:.2f}$".format(r_value)
...
plot_scatter(gt_tsts, pred_tsts, "Total Sleep Time", "Model Prediction", "Total Sleep Time (TST)",
...
plot_scatter(gt_sleep_efficiencies, pred_sleep_efficiencies, "Sleep Efficiency (%)",
             ...
             tick_interval=15)
plot_scatter(gt_sleep_onset_latencies, pred_sleep_onset_latencies, "Sleep Onset Latency", "Model Prediction",
             "SOL", xlim=[0, 6], ylim=[0, 6], xy_text=(1, 5), tick_interval=1)
plot_scatter(gt_wake_durations, pred_wake_durations, "WASO", "Model Prediction",

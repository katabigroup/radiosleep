...
def plot_confusion_matrix(cm_stack, classes, fig_name, cmap=plt.cm.Blues, fontsize = 10):
    ...
    plt.ylabel('PSG Labels', fontsize = fontsize)
    plt.xlabel('Predictions label', fontsize = fontsize)

    # set figsize
    plt.gcf().set_size_inches(8, 6)

    plt.tight_layout(pad = 0.1)
    plt.savefig("figures/graph_abstract/%s.png"%(fig_name), dpi = 300)
    plt.close()
...
plot_confusion_matrix(five_class_recs, five_classes_name, fig_name, cmap = "Blues", fontsize = 26)

four_classes_name = ["Wake", "Light", "Deep", "REM"]
fig_name = "fig2c_four_class_recs_cm"
plot_confusion_matrix(four_class_recs, four_classes_name, fig_name, cmap = "Blues", fontsize = 26)